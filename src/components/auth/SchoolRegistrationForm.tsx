  ...
  async function onSubmit(values: SchoolRegistrationFormValues) {
    ...
    try {
      ...
      const userProfileData: Omit<UserProfile, 'uid'> & { createdAt: FieldValue; updatedAt: FieldValue; schoolId: string; } = {
        ...
        status: 'approved', // First admin of a school is auto-approved
      ...
      if (error.code === 'auth/email-already-in-use') {
        errorMessage = t('errorEmailInUse', 'This email address is already in use by another account.');
        form.setError("adminEmail", { type: "manual", message: errorMessage });
      } else if (error.code === 'auth/weak-password') {
        errorMessage = t('errorWeakPassword', 'The password is too weak. It must be at least 6 characters long.');
        form.setError("adminPassword", { type: "manual", message: errorMessage });
      }
      toast({
