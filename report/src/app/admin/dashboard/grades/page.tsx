...
=======
  ...
  const headers = [
    "Student Name", "Student System ID", "Course Code", "Course Name", 
    "CA1", "CA2", "Exam", "Total Marks", "Term", "Status", "Remarks", 
    "Entered By", "Date Recorded"
  ];

  const rows = grades.map(grade => [
    grade.studentName,
    studentSystemIdMap.get(grade.studentId) || 'N/A', // Get studentSystemId
    grade.courseName.split('(')[1]?.replace(')','').trim() || 'N/A', 
    grade.courseName.split('(')[0].trim(),
    grade.ca1 ?? '',
    grade.ca2 ?? '',
